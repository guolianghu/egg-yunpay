...
function getSign(str, sign_method, key = '', output = 'base64') {
    switch (sign_method) {
        case 'rsa':
            return getRsaSign(str, key, output);
            break;
...
class YunPay {
     ...
     */
    _getSign(body) {
        return getSign(body + this.secret, this.secret, this.sign_method);
    }
    ...
    _sendRequest(service, data) {
        ...
        var  body1 = _.extend({}, data, {service}, {terminalType});
