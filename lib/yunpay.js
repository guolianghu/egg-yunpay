...
class YunPay {
     ...
     * @param describe 付款原因
     * @param payer 付款人
     * @param notifyUrl 支付结果（后台）通知地址
     * @param returnUrl 付款完成（前台）跳转地址
     * @param orderNO 订单号（与业务订单关联的信息）
     * @param expiryTime 过期时间(默认：1天)
     *
     * @return {Promise}
     */
    createPaymentOrder(businessId, traceNO, amount, describe, payer, notifyUrl, returnUrl, orderNO, expiryTime = Date.now() + 3600*24) {
        ...
        return this._sendRequest('acquirer', params);
    }
